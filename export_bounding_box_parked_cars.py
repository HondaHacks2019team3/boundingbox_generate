...
def get_2d_boxes(sample_data_token: str, visibilities: List[str]) -> List[OrderedDict]:
    ...
    ann_recs = [ann_rec for ann_rec in ann_recs if (ann_rec['category_name'] == "vehicle.car")]
    ...
    ann_recs = [ann_rec for ann_rec in ann_recs if (parked_token in ann_rec['attribute_tokens'] or stopped_token in ann_rec['attribute_tokens'])]
    ...
    for ann_rec in ann_recs:
        ...
        repro_rec = [vehicle_state, (max_x + min_x)/(2 * 1600), (max_y + min_y)/(2*900), (max_x - min_x)/(1600), (max_y - min_y)/(900)] 
...
def main(args):
    ...
    #find token for scenes
    scenes = ['scene-0655', 'scene-0103','scene-0553','scene-0757']
    scene_tokens = [scene['token'] for scene in nusc.scene if scene['name'] in scenes]
    # Get tokens for all camera images.
    sample_data_camera_tokens = [s['token'] for s in nusc.sample_data if (s['channel'] == 'CAM_FRONT_RIGHT') and
                                 s['is_key_frame'] and (nusc.get('sample',s['sample_token'])['scene_token'] in scene_tokens)]
    print(len(sample_data_camera_tokens))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default='/home/richard/data/sets/nuscenes', help="Path where nuScenes is saved.")
    ...
    parser.add_argument('--scene', type=str, default=['0', '1', '2', '3', '4'],
                        help='scenes, the higher the number the higher the visibility.', nargs='+')
